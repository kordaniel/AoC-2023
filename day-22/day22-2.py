            ...
            for x in range(brick_lower.min_x, brick_lower.max_x+1):
                for y in range(brick_lower.min_y, brick_lower.max_y+1):
                    if brick_upper.min_x <= x <= brick_upper.max_x and brick_upper.min_y <= y <= brick_upper.max_y:
                        if not brick_upper.name in supported_by_bricks:
                            supported_by_bricks[brick_upper.name] = set()

                        brick_supports[brick_lower.name].add(brick_upper.name)
                        supported_by_bricks[brick_upper.name].add(brick_lower.name)
                        support_found = True
                        break
                if support_found:
                    break

    return brick_supports, supported_by_bricks


def filter_falling_bricks(
        brick: Union[str, int],
        brick_supports: Dict[Union[str, int], Set[Union[str, int]]],
        supported_by_bricks: Dict[Union[str, int], Set[Union[str, int]]]
) -> Set[Union[str, int]]:
    if all(len(supported_by_bricks[bricks]) > 0 for bricks in brick_supports[brick]):
        return set()

    fallen_bricks = set()

    for supported_brick in brick_supports[brick]:
        if len(supported_by_bricks[supported_brick]) == 0:
            fallen_bricks.add(supported_brick)
            for bricks in supported_by_bricks.values():
                bricks.discard(supported_brick)
        fallen_bricks.update(filter_falling_bricks(supported_brick, brick_supports, supported_by_bricks))

    return fallen_bricks


descended_bricks = descend_bricks(parse_input(inp))
brick_supports, supported_by_bricks = construct_bricks_graph(descended_bricks)

cnt_bricks_fallen = 0
for supporting_brick in brick_supports:
    supported_cpy = copy.deepcopy(supported_by_bricks)
    for supporters in supported_cpy.values():
        supporters.discard(supporting_brick)

    cnt_bricks_fallen += len(filter_falling_bricks(supporting_brick, brick_supports, supported_cpy))


print('Part 2:', cnt_bricks_fallen)
